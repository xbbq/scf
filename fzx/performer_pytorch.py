...
class Performer(nn.Module):
    def __init__(
        ...
        super().__init__()
        print('decoder dim',dim)
        #创建一个 nn.ModuleList 来存储模型的层。
        layers = nn.ModuleList([])
        #将 local_attn_heads 转换为一个元组，并将其扩展到与 depth 相同的长度。
        ...
        #根据是否使用 ScaleNorm、ReZero 或 PreLayerNorm 选择包装函数。
        if use_scalenorm:
        ...
        #遍历每一层，添加 SelfAttention 和 FeedForward 层到 layers 中。
        for _, local_heads in zip(range(depth), local_attn_heads):
            ...
            #如果 cross_attend 为 True，则添加额外的 SelfAttention 层。
            if not cross_attend:
                continue
            layers.append(nn.ModuleList([
        ...
        #创建模型 self.net，它是一个可逆或顺序的序列，包含所有层。
        self.net = execute_type(layers, args_route = {**attn_route_map, **context_route_map})

        # keeping track of when to redraw projections for all attention layers
        #记录投影矩阵重绘的频率和自动检查重绘的标志。
        self.auto_check_redraw = auto_check_redraw
        self.feature_redraw_interval = feature_redraw_interval
        #创建一个 calls_since_last_redraw 缓冲区，用于跟踪自上次重绘以来的调用次数。
    ...
    #调用 check_redraw_projections 并将输入传递给 self.net。
